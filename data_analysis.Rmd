...
library(AER)
iv1 <- ivreg(
  log(TCC) ~ SALARY_HISTORY*GENDER + YRS_EXP + highest|
    SHB*GENDER + YRS_EXP + highest,
...
```{r}
# ols model, no log adjustment, small control set, with interaction
lm1 <- lm(
  log(TCC) ~ SALARY_HISTORY*GENDER + YRS_EXP + highest,
  # using responses in prof adlers example
  data = filter(
...
talk to statlab about projecting X onto Z
talk to harry too haha


senior ideas: geographic variation (job moves during pandemic), occupational status and gender
