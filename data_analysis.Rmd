...
salaryhist2 <- function(x){
  x %>%
    filter(!is.na(GENDER)) %>%
  ggplot() +
    geom_density(aes(x = TCC, fill = GENDER),
                 alpha=0.25) + 
    theme(legend.position = c(0.1, .73), # adding legend
          plot.title = element_text(vjust = 0.25)) + # moving title
    xlim(c(0,200000)) +
...
table(filter(df_2016, year(df_2016$UPDATE_DATE) == 2016)$GENDER)
sum(is.na(filter(alldata,year(alldata$UPDATE_DATE)==2016)$TCC))
nrow(filter(alldata,year(alldata$UPDATE_DATE)==2016))
```
